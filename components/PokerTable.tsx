...
const PokerTable: React.FC<PokerTableProps> = ({
  ...
  // Strict control: ONLY Scrum Master
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-4 relative overflow-hidden min-h-[50vh]">
      ...
      {/* Table Surface */}
      {/* Responsive aspect ratio: Square-ish on mobile, Wide oval on desktop */}
      <div className="relative w-full max-w-4xl aspect-square md:aspect-[2/1] bg-slate-800/50 rounded-full border-8 border-slate-700 shadow-2xl flex items-center justify-center transition-all">
        ...
        {seatedUsers.map((user, index) => {
          ...
          // Responsive Radii
          // Mobile: Use smaller radius (closer to center) to avoid edge cutoff
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col items-center">
                <span className="text-[10px] md:text-xs text-slate-200 font-medium bg-slate-900/90 px-2 py-0.5 rounded-full truncate max-w-[80px] md:max-w-[100px] border border-slate-700 shadow-sm">
                    {user.name}
                </span>
              </div>
            </div>
